! to be added in modules.f90
#ifdef MAGNETIC
!=================================================================================================
!=================================================================================================
! module_ferro: Contains variables for ferromagnetic forces
!-------------------------------------------------------------------------------------------------
module module_ferro
  real(8), allocatable, dimension(:,:,:) :: phimag,chip1
!  real(8), parameter :: mu_0 = 1.256637d-6 ! vacuum value
  real(8) :: mu1mag,mu2mag
  character(len=6) :: phi_init_type
  real(8):: phi_init_param
...
subroutine init_mag
  ...
  call phimaginitialize(phi_init_type,phi_init_param)
...
subroutine ReadMagParameters
  ...
  open(unit=10, file='inputferromag', status='old', action='read', iostat=ierr)
  if (ierr .ne. 0)
    call err_no_out_dir("ReadMagParameters: error opening 'inputferromag' file --- perhaps it does not exist ?")
...
subroutine phimaginitialize(phi_init_type,phi_init_param) 
  ...
  integer :: i,j,k
    ...
    if phi_init_type='linear'
      do k=kmin,kmax-1;  do j=jmin,jmax-1;  do i=imax,imax-1
        phimag(i,j,k) = 1.0d0 + phi_init_param*z(k)
      enddo;  enddo;  enddo
    else
